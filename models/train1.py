...
def train(train_iter, dev_iter, model, args, model_name):
    ...
    for epoch in range(1, args.epochs+1):
        for batch in train_iter:
            left, right, aspect, target = batch.left, batch.right, batch.aspect, batch.label
            ...
            # print(feature)
            if args.cuda:
                left, right, aspect, target = left.cuda(), right.cuda(), aspect.cuda(), target.cuda()

            optimizer.zero_grad()
            logit = model(left, right, aspect)
            ...
            if steps % args.log_interval == 0:
                ...
                sys.stdout.write(
                    '\r{}-Batch[{}] - loss: {:.6f}  acc: {:.4f}%({}/{})'.format(model_name, steps,
                                                                             # loss.data[0],
                                                                             loss.item(),
                                                                             accuracy,
                                                                             corrects,
            ...
            if steps % args.test_interval == 0:
                ...
                if dev_acc > best_acc:
                    ...
                    last_step = steps
                    if args.save_best:
                        save(model, model_name, args.save_dir, 'best', steps)
                else:
                    if steps - last_step >= args.early_stop:
                        print('early stop by {} steps.'.format(args.early_stop))
            elif steps % args.save_interval == 0:
                save(model, model_name, args.save_dir, 'snapshot', steps)
