...
class ATAE_LSTM(nn.Module):
    def __init__(self, args):
        ...
        self.lstm = nn.LSTM(self.args.embed_dim*2, self.args.hidden_size, self.args.num_layers, batch_first=True)
    ...
    def forward(self, x, aspect):
        ...
        # print(aspect.size())
        aspect = aspect.sum(1)   # 通过求和得出aspect embedding
        # print(x.size(),aspect.size())
        x=torch.cat([x, torch.unsqueeze(aspect, 1).expand(x.size(0),x.size(1),x.size(2))] ,dim=2)
